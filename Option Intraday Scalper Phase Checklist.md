...
- [ ] Set the initial paper-account balance.
- [ ] Define the simulated commission schedule.
- [ ] Define simulated regulatory fees.
- [ ] Define default option-entry and stop slippage.
- [ ] Define conservative fill behavior when displayed option size is unavailable.
- [ ] Set the underlying quote stale threshold; initial recommendation: 5 seconds.
- [ ] Set the option quote stale threshold; initial recommendation: 3 seconds.
- [ ] Set the completed-candle grace period.
- [ ] Obtain and configure Public API credentials.
- [ ] Choose local, staging, and production deployment environments.
- [ ] Configure PostgreSQL connection settings.
- [ ] Decide whether Alpaca comparison mode will be implemented.
- [ ] Document the expected Webull API approval path and timing.
...
- [ ] Add Python formatting, linting, and type-checking configuration.
- [ ] Add React/TypeScript formatting, linting, and type-checking configuration.
- [ ] Add backend and frontend test frameworks.
- [ ] Add database migration tooling.
- [ ] Add structured logging conventions.
- [ ] Add UTC storage and Eastern Time display conventions.
- [ ] Define shared enums for order states, signal states, exit reasons, and execution modes.
- [ ] Define a versioned API contract between backend and frontend.

## 0.3 Initial Architecture Contracts

- [ ] Define the `MarketDataProvider` interface.
- [ ] Define the `ExecutionBroker` interface.
- [ ] Define canonical models for bars, quotes, expirations, option contracts, and option chains.
- [ ] Define canonical models for orders, replacements, fills, positions, account state, and cash events.
- [ ] Ensure strategy components cannot import provider-specific execution code.
- [ ] Define idempotency rules for order events, fills, and ledger transactions.
- [ ] Define application startup recovery behavior.

### Phase 0 Completion Gate

- [ ] All unresolved Version 1 settings have documented defaults.
- [ ] Backend, frontend, database, tests, and migrations run locally.
- [ ] Broker-neutral interfaces and canonical data models are approved.
